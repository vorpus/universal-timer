...
import type { Settings, TimerEvent, TimerState, TimerInfo, TimelineSegment, TimelineData, ExportData, OperationResult, AppError } from '../shared/types'
...
// Tray icon paths
const TRAY_ICONS_PATH = getResourcePath('tray-icons')

function createTrayIcon(timerIndex: number | null): Electron.NativeImage {
  let iconPath: string
...
function updateTrayIcon(timerIndex: number | null): void {
  if (tray) {
...
function playSound(soundType: 'start' | 'pause'): void {
...
let runningTimer: string | null = null // Name of the currently running timer (normalized)
let runningTimerStart: number | null = null // Timestamp when it started
...
function notifyError(message: string, details: Error | string | null = null): void {
  console.error(message, details)
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('app:error', { message, details: (details instanceof Error ? details.message : details) })
  }
}

function appendEvent(event: TimerEvent): void {
  try {
...
function purgeTimerEvents(normalizedName: string): void {
  try {
...
function loadEvents(): TimerEvent[] {
  try {
    const eventsPath = getEventsPath()
...
const timerDisplayNames = new Map<string, string>()

function getDisplayName(normalizedName: string, originalName: string): string {
...
function calculateTotalForDay(events: TimerEvent[], dayStart: Date, dayEnd: Date, activeTimers: Map<string, number> | null, now: number | null): number {
  // Build intervals from events
  const timerIntervals = new Map<string, { start: number; end: number }[]>()
  const tempActiveTimers = new Map<string, number>()

  for (const event of events) {
    const ts = event.ts

    if (event.event === 'start') {
      tempActiveTimers.set(event.timer, ts)
    } else if (event.event === 'pause') {
      if (tempActiveTimers.has(event.timer)) {
        const startTs = tempActiveTimers.get(event.timer)!
        if (!timerIntervals.has(event.timer)) {
          timerIntervals.set(event.timer, [])
        }
        timerIntervals.get(event.timer)!.push({ start: startTs, end: ts })
        tempActiveTimers.delete(event.timer)
      }
...
function calculatePerTimerTotalsForDay(events: TimerEvent[], dayStart: Date, dayEnd: Date, activeTimers: Map<string, number> | null, now: number | null): Map<string, number> {
  // Build intervals from events
  const timerIntervals = new Map<string, { start: number; end: number }[]>()
  const tempActiveTimers = new Map<string, number>()
...
function calculatePerTimerWeeklyStats(events: TimerEvent[], activeTimers: Map<string, number>, now: number): { weeklyTotals: Map<string, number>; weeklyTrends: Map<string, number> } {
  ...
  const previousDayTotals = new Map<string, number[]>() // timer -> [dayTotals...]
...
function calculateWeeklyTrend(events: TimerEvent[], todayTotal: number, activeTimers: Map<string, number>, now: number): number {
  const today = getDayStart()
...
function computeTimerState(): TimerState {
  ...
  const timerIntervals = new Map<string, { start: number; end: number }[]>() // timer -> [{start, end}]
  const activeTimers = new Map<string, number>() // timer -> startTs (for unterminated timers)
...
function getTodayTimeline(): TimelineData {
  const events = loadEvents()
  const now = Date.now()
  const todayStart = getDayStart().getTime()
  const todayEnd = getDayEnd(getDayStart()).getTime()
  ...
  const timerOrder: string[] = [] // Track order of first appearance
  ...
  for (const event of events) {
    ...
    if (event.event === 'start') {
      activeTimers.set(event.timer, ts)
      if (event.timer && !timerOrder.includes(event.timer)) {
...
function getTrayIconIndex(state: TimerState): number | null {
